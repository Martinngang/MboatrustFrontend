...
export function MilestoneSubmitScreen() {
  ...
  const submit = async () => {
    if (isOnline) {
      submitMilestoneProof(project.id, milestone.id)
      setStep('submitted')
      return
    }
    await enqueue({
        ...
        <div className="flex flex-col items-center justify-center h-full px-8 text-center">
          <div className="w-20 h-20 rounded-full flex items-center justify-center mb-6" style={{ background: '#EFF6FF' }}>
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
              <path d="M18 4V22M12 10L18 4L24 10" stroke="#3B82F6" strokeWidth="2.5" strokeLinecap="round" />
              <path d="M8 28H28" stroke="#3B82F6" strokeWidth="2" strokeLinecap="round" />
            </svg>
          </div>
      ...
      <AppShell noNav>
        <div className="flex flex-col items-center justify-center h-full px-8 text-center">
          <div className="w-20 h-20 rounded-full flex items-center justify-center mb-6" style={{ background: '#FFF7E6' }}>
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
              <circle cx="18" cy="18" r="3" fill="#E8A020" />
              <circle cx="18" cy="18" r="11" stroke="#E8A020" strokeWidth="2" strokeDasharray="4 4" />
            </svg>
          </div>
          ...
          <div className="mb-6 inline-flex items-center gap-2 rounded-full px-3 py-1.5" style={{ background: '#FFF7E6' }}>
            <span className="h-1.5 w-1.5 rounded-full" style={{ background: '#E8A020' }} />
            <span style={{ fontFamily: FONT.mono, color: '#92400E' }} className="text-[9px] uppercase tracking-wider">Pending sync</span>
          </div>
          {isOnline && (
  ...
  return (
    <AppShell noNav>
      ...
      <div className="px-5 py-5 space-y-5 overflow-y-auto sm:mx-auto sm:max-w-2xl">
        {/* Milestone card */}
        <div className="rounded-2xl border-2 p-4" style={{ borderColor: C.forest, background: '#F0FDF4' }}>
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: '#EFF6FF' }}>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M8 2C5.8 2 4 3.8 4 6C4 9.5 8 14 8 14C8 14 12 9.5 12 6C12 3.8 10.2 2 8 2Z" stroke="#3B82F6" strokeWidth="1.3" />
        ...
        <div className="rounded-xl p-4 border" style={{ background: '#FFF7E6', borderColor: '#F5DCA0' }}>
          <div style={{ fontFamily: FONT.mono, color: '#92400E' }} className="text-[10px] uppercase tracking-widest mb-2">After submission</div>
          <div className="space-y-2">
            {['A local verifier reviews the evidence on-site (72h)', 'The funder receives your proof for approval', 'Funds are released once approved'].map((s, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <span style={{ fontFamily: FONT.sans, color: '#78350F' }} className="text-xs">{s}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-5 pb-8 pt-4 border-t sm:mx-auto sm:max-w-2xl" style={{ borderColor: C.parchmentDark, background: C.white }}>
        ...
        <PillButton onClick={submit} fullWidth disabled={photos.length === 0}>
          {photos.length === 0 ? 'Add at least one photo' : isOnline ? 'Submit milestone proof' : 'Save for sync'}
        </PillButton>
      </div>
    </AppShell>
  )
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {['all', 'active', 'completed'].map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              style={{ fontFamily: FONT.mono, background: filter === f ? C.forest : C.parchment, color: filter === f ? C.white : C.inkMuted }}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div className="px-5 py-4 space-y-4 sm:grid sm:grid-cols-2 sm:gap-4 sm:space-y-0">
        {filtered.length === 0 ? (
          <div className="col-span-full text-center py-16">
            ...
            <div style={{ fontFamily: FONT.serif }} className="text-lg font-bold mb-1">No projects yet</div>
            <div style={{ fontFamily: FONT.sans, color: C.inkMuted }} className="text-sm mb-6">Create a funding request or wait for a funder to assign you a project.</div>
            <PillButton onClick={() => nav('/funder/create')} fullWidth>Create a request</PillButton>
          </div>
        ) : filtered.map((p) => {
            ...
            <Card key={p.id} onClick={() => nav(`/funder/project/${p.id}`)}>
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  {pendingSync ? (
                    <span className="inline-flex items-center gap-1 rounded-full px-2 py-1" style={{ background: '#FFF7E6' }}>
                      <span className="h-1.5 w-1.5 rounded-full" style={{ background: '#E8A020' }} />
                      <span style={{ fontFamily: FONT.mono, color: '#92400E' }} className="text-[9px] uppercase tracking-wider font-bold">Pending sync</span>
                    </span>
                  ) : (
  ...
  return (
    <AppShell noNav>
      ...
      <div className="px-5 py-5 space-y-5 sm:mx-auto sm:max-w-2xl">
        <div className="rounded-2xl border-2 p-4" style={{ borderColor: C.amber, background: '#FFF7E6' }}>
          <div style={{ fontFamily: FONT.mono, color: '#92400E' }} className="text-[10px] uppercase tracking-widest mb-1">Under review</div>
          <div style={{ fontFamily: FONT.serif }} className="font-bold">{milestone.title}</div>
