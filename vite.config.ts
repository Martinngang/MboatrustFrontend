...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      VitePWA({
        // The SW only ships in production builds — devOptions stays disabled so
        // the live Figma Make preview (which runs the dev server) is never
        // caught behind a stale cached response while iterating.
        devOptions: { enabled: false },
        registerType: 'autoUpdate',
        injectRegister: false,
        // injectManifest (not the default generateSW) because the offline evidence
        // queue needs a hand-written `sync` event listener for Background Sync —
        // see src/sw.ts, which owns precaching + runtime caching + the sync handler.
        ...
        manifest: {
          ...
          description: 'Diaspora funding, contractor and land escrow platform for Cameroon — verified projects, milestone-based payments, and on-the-ground evidence verification.',
          theme_color: '#1A4731',
          background_color: '#121510',
          display: 'standalone',
        ...
        // injectManifest strategy: only controls what gets fed into self.__WB_MANIFEST
        // inside src/sw.ts. Runtime caching (fonts, project images) and the
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Mboa Trust"
